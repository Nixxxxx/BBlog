	...
	public String watermark() throws Exception {
		...
		if (image != null && image.length > 0) {
			for (int i = 0; i < image.length; i++) {
				...
				pi.setImageURL(uploadService.uploadImage(image[i], imageFileName[i], uploadPath, realUploadPath));

				// 水印添加操作
				pi.setLogoImageURL(markService.watermark(image[i], imageFileName[i], uploadPath, realUploadPath));
